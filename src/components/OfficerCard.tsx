  ...
  return (
    <div className="flex gap-5 border-b border-border py-4">
      <div className="flex flex-col items-center gap-1.5 shrink-0">
        {officer.imagen ? (
          <img src={officer.imagen} alt={officer.nombre} loading="lazy" width={112} height={128} className="w-28 h-32 object-cover border border-gold" />
        ) : (
          <SIN_FOTO />
        )}
      ...
      <div className="flex flex-col justify-center gap-1 text-sm min-w-0">
        ...
        <div className="mt-2">
          <span className="text-muted-foreground text-xs">NOTAS: {officer.notas}</span>
        </div>
      </div>
    </div>
  );
